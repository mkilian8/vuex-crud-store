...
exports.del = exports.post = exports.put = exports.get = exports.moduleOptions = undefined;
...
var moduleOptions = exports.moduleOptions = {
  errorMessageCallback: function errorMessageCallback(m) {
    console.log(m);
  }

  //API calls
};var apiFetch = function apiFetch(uri, opt) {
  opt.credentials = 'include';
  if (opt.showMsg == undefined) opt.showMsg = true;
  return fetch(uri, opt).then(function (r) {
    return _promise2.default.all([r.status, r.json()]);
  }).then(function (_ref) {
    var _ref2 = (0, _slicedToArray3.default)(_ref, 2),
        code = _ref2[0],
        json = _ref2[1];

    if (code != 200) return _promise2.default.reject({ code: code, json: json });else return json;
  }).catch(function (obj) {
    var m = obj.message;
    if (obj.json) m = obj.json.message;else m = opt.method + ' ' + uri + ' failed';
...
var del = exports.del = function del(uri) {
  var opt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
...
exports.createCrudStore = undefined;
...
var defaultUiState = {
  ...
  getters: {
    endpoint: function endpoint(state, getters, rootState) {
      return function (id) {
        if (!id) return 'http://collection';
...
var createCrudStore = exports.createCrudStore = function createCrudStore() {
  var ui_state = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : defaultUiState;
  var opt = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  // Assign API options
  (0, _assign2.default)(_api.moduleOptions, defaultOptions, opt);
  ...
  var mutations = {
    RECEIVE_RESOURCE: function RECEIVE_RESOURCE(state, _ref) {
      var resource = _ref.resource;

      var idAttr = state.ui_state.id_key;
      var old_resource = state.collection.find(function (item) {
        return item[idAttr] == resource[idAttr];
      });
      if (!old_resource) return state.collection.push(resource);
      state.collection = state.collection.map(function (item) {
        if (item[idAttr] == resource[idAttr]) return resource;
        return item;
      });
    },
    RECEIVE_COLLECTION: function RECEIVE_COLLECTION(state, _ref2) {
      var resources = _ref2.resources;

      state.collection = resources;
    },
    DELETE_RESOURCE: function DELETE_RESOURCE(state, _ref3) {
      var id = _ref3.id;
  ...
  var actions = {
    create: function create(_ref4, _ref5) {
      ...
      console.log('CREATE');
      var endpoint = getters.endpoint();
      var json = (0, _stringify2.default)(resource);
      return (0, _api.post)(endpoint, {
        body: json
      }).then(function (resource) {
        commit('RECEIVE_RESOURCE', { resource: resource });
        return resource;
      });
    },
    read: function read(_ref6) {
      ...
      console.log('READ');
      var endpoint = getters.endpoint(id);
      return (0, _api.get)(endpoint).then(function (resource) {
        if (resource.resources) commit('RECEIVE_COLLECTION', { resources: resource.resources });else commit('RECEIVE_RESOURCE', { resource: resource });
      });
    },
    update: function update(_ref8, _ref9) {
      ...
      console.log('UPDATE');
      var id = resource[state.ui_state.id_key];
      var endpoint = getters.endpoint(id);
      var json = (0, _stringify2.default)(resource);
      return (0, _api.put)(endpoint, {
        body: json
      }).then(function (resource) {
        commit('RECEIVE_RESOURCE', { resource: resource });
      });
    },
    del: function del(_ref10, _ref11) {
      ...
      var id = resource[state.ui_state.id_key];
      var endpoint = getters.endpoint(id);
      return (0, _api.del)(endpoint).then(function (result) {
        commit('DELETE_RESOURCE', { id: id });
      });
    }
  };
...
var $          = __webpack_require__(3)
  ...
  , global     = __webpack_require__(2)
  , ctx        = __webpack_require__(6)
  , classof    = __webpack_require__(10)
  , $export    = __webpack_require__(13)
  , isObject   = __webpack_require__(8)
  , anObject   = __webpack_require__(4)
  , aFunction  = __webpack_require__(9)
...
if(!USE_NATIVE){
  ...
  __webpack_require__(52)(P.prototype, {
